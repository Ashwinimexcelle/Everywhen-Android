package com.mexcelle.thoughtifydemo.repository

import android.content.Context
import android.util.Log
import androidx.lifecycle.MutableLiveData
import com.mexcelle.thoughtifydemo.model.ChangePasswordInputData
import com.mexcelle.thoughtifydemo.model.ChangePasswordResponseData
import com.mexcelle.thoughtifydemo.model.OTPInputData
import com.mexcelle.thoughtifydemo.model.OTPResponseData
import com.mexcelle.thoughtifydemo.retrofit.RetrofitClient
import com.mexcelle.thoughtifydemo.util.Constants
import com.mexcelle.thoughtifydemo.util.Utility
import org.json.JSONObject
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

object ChangePasswordRepository {

    val changePasswordResponseData = MutableLiveData<ChangePasswordResponseData?>()

    fun changePassword(context: Context, changePasswordInputData: ChangePasswordInputData): MutableLiveData<ChangePasswordResponseData?> {

        val call = RetrofitClient.apiInterface.changePassword(Constants.AUTH_TOKEN,Constants.USER_AUTHTOKEN,changePasswordInputData)
        call.enqueue(object: Callback<ChangePasswordResponseData> {
            override fun onFailure(call: Call<ChangePasswordResponseData>, t: Throwable) {
                // TODO("Not yet implemented")
                Log.e("DEBUG : ", t.message.toString())
            }

            override fun onResponse(
                call: Call<ChangePasswordResponseData>,
                response: Response<ChangePasswordResponseData>
            ) {

                if(response.body()!= null){

                    if(response.body().toString() !=null){

                        val data = response.body()
                        changePasswordResponseData.value = data!!

                    }else{

                        Utility.hideProgressDialog(context)
                        Utility.showDialog( context,"Error !!","Server Error.","Ok","Cancel")
                    }
                }else{

                    val jsonObject = JSONObject(response.errorBody()?.string())
                    var msg: String = ""
                    if(jsonObject.has("message")){

                        msg = (jsonObject.getString("message"))
                        Utility.hideProgressDialog(context)
                        Utility.showDialog( context,"Error !!",""+msg,"Ok","Cancel")

                    }else{

                        Utility.hideProgressDialog(context)

                        Utility.showDialog( context,"Error !!","Server Error.","Ok","Cancel")

                    }


                }

            }
        })

        return changePasswordResponseData
    }
}