...
class Utility private constructor() {
    ...
    companion object {
        ...
        fun getTime(publishDate: String): String  {

            val inputFormat = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'")
            val outputFormat = SimpleDateFormat("HH:mm:SSS")
            val date :Date = inputFormat.parse(publishDate)

            val formattedPublishDate = outputFormat.format(date)
            //val publishDate :Date = outputFormat.parse(formattedPublishDate)
            val currentTime = Calendar.getInstance().time
            val currentFormattedTime = outputFormat.format(currentTime)

            val publishDate :Date = outputFormat.parse(formattedPublishDate)
            val currentDate :Date = outputFormat.parse(currentFormattedTime)


            //val formattedDate = outputFormat.format(date)

            val millis: Long = publishDate.time - currentDate.time
            ...
            /*  var time :String = ""
              ...
              Log.e("time ","formattedDate "+formattedDate)

              Log.e("time ","time "+date.hours)
              Log.e("time ","time "+date.minutes)
        ...
        fun setExodar(context: Context,editText: TextView) {

            val tf = Typeface.createFromAsset(context.getAssets(), "fonts/exodar.ttf")
            editText.setTypeface(tf)
        ...
        fun setSolidFontAwesome1(context: Context,editText: TextDrawable) {
